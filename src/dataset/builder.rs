...
impl DatasetBuilder {
    ...
    pub fn from_rows(features: &[f64], n_rows: usize, n_features: usize, labels: &[f32], config: &Config) -> Result<Dataset> {
        ...
        // Transpose to column-major raw values, then bin per column in parallel.
    ...
    pub fn from_columns(columns: Vec<Vec<f64>>, labels: &[f32], config: &Config) -> Result<Dataset> {
