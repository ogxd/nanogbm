...
fn make_classification(train_n: usize, valid_n: usize, d: usize, seed: u64) -> (Vec<f64>, Vec<f32>, Vec<f64>, Vec<f32>) {
...
fn trains_and_reduces_logloss_on_synthetic() {
    ...
    let model = GbdtTrainer::new(&cfg).fit(&train_ds, Some(&valid_ds)).unwrap();
    ...
    assert!(final_loss < init_loss * 0.9, "model failed to improve appreciably: {init_loss} -> {final_loss}");
...
fn predict_bin_and_raw_paths_agree() {
    ...
    // bin-encoded prediction: bin then traverse using predict_on_dataset
    ...
    // The two should agree closely (the only difference is float representation of
    // raw vs binned threshold comparisons).
    let max_abs = raw.iter().zip(bin_raw.iter()).map(|(a, b)| (a - b).abs()).fold(0.0f64, f64::max);
    assert!(max_abs < 1e-9, "raw vs bin predictions diverged by {max_abs}");
}
