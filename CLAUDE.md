...
1. **Bucket all feature values once, up front** (`dataset::bin_mapper`, `dataset::builder`). Real-valued columns are slow to split on — you'd have to consider every distinct value as a possible split point. So before training, each feature column is bucketed into at most ~255 buckets using quantiles, and the raw `f64` is replaced by a small `u16` bucket id. From here on the trees only ever look at bucket ids. Bucket **0 is reserved for "missing" (NaN)**. Columns listed in `Config::cat_features` are bucketed differently: distinct integer values (cast from `f64 as i64`) become their own bins, capped at `max_cat_bin` (low-frequency values collapse to MISSING). The mapping (`BinMapper`) is fit on training data and **reused as-is** for validation and inference — never refit it. The `Dataset` is stored **column-major** because every hot loop iterates one feature across many rows.
